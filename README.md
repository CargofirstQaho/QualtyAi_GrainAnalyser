...
**Qualty AI** is a web application built with Streamlit for performing visual grain quality analysis using Google's Gemini AI model. Users can upload an image or take a photo of a grain sample (Rice, Wheat, Barley, Corn, Oats, Sorghum), and the application provides a detailed analysis including grain identification, quality assessment, defect detection, overall grading, and usage recommendations.
...
This section documents concrete details in app.py so the README matches the code.

### Entry point
- Launch with: `streamlit run app.py`.
- Main UI organized into three tabs: Analysis, History, Settings.

### Important constants (set at top of script)
- GEMINI_MODEL — model id used for inference.
- GRAIN_OPTIONS — supported grain types list.
- SUPPORTED_IMAGE_TYPES — allowed upload extensions.
- HISTORY_LIMIT — default max items kept in history.
- Environment variable: `GOOGLE_API_KEY` (loaded from `.env`).

### Key helper functions
- generate_prompt(image_bytes, selected_grain): builds the structured JSON prompt sent to Gemini.
- process_image(image_bytes, selected_grain): calls the Gemini API, handles API errors and content filtering, extracts JSON from model output.
- save_to_history(result_obj, image_bytes): writes a compact history entry with thumbnail and timestamp; enforces HISTORY_LIMIT.
- render_analysis_results(parsed_json): renders the AI response into Streamlit containers and metric cards.
- render_progress_circle(score): returns HTML/CSS used to display circular progress indicators.
- get_color_from_score(score) / get_color_from_grade(grade): map numeric/grade values to UI color classes.

### Session state usage
- st.session_state stores current image, parsed results, history list, selected grain, UI flags, and last error message — enabling persistent behavior across reruns.

### Dependencies
- streamlit
- google-generativeai (or equivalent Google Generative AI SDK used in app.py)
- pillow (PIL)
- python-dotenv
...
### Troubleshooting
- "No API key" message: ensure `GOOGLE_API_KEY` is present and Streamlit restarted.
